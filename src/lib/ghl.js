...
const normalizeEvent = (record) => {
  ...
  const startDateStr = props.event_start_date ?? props.event_date ?? ''

  const image = Array.isArray(props.event_image) && props.event_image[0]?.url
  ...
  return {
    ...
    date: parseDate(startDateStr) ?? EMPTY_DATE,
...
export const fetchGHLEvents = async () => {
  try {
    ...
    if (!res.ok) {
      console.error('[GHL Events] search failed:', res.status, await res.text())
      return []
    }
